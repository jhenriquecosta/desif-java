...
public class VerificaRegistroBanco
{
        ...
        throws Exception
    {
        ...
        if(!declaracao.getCnpjResponsavelRecolhimento().equals(""))
        {
            ...
            if(dependencias.size() < 1)
            {
                ...
                regUtil.setErro(declaracao.getLinhaIdentificacaoDeclaracao().intValue(), "ED032", coluna, (short)2, registro, txtSolucao);
            } else
            {
                List dep = dependenciaDao.findField("cnpjProprio", declaracao.getCnpjResponsavelRecolhimento());
                if(dep.size() == 1)
                    if(((IdentificacaoDependencia)dep.get(0)).getContabilidadePropria().shortValue() == 1)
                    {
                        if(declaracao.getCidade().longValue() != 0x2f6598L)
                        {
                            ...
                            regUtil.setErro(declaracao.getLinhaIdentificacaoDeclaracao().intValue(), "ED033", coluna, (short)2, registro, txtSolucao);
                        }
                    } else
                    if(((IdentificacaoDependencia)dep.get(0)).getContabilidadePropria().shortValue() == 2)
                    {
                        ...
                        regUtil.setErro(declaracao.getLinhaIdentificacaoDeclaracao().intValue(), "ED019", coluna, (short)2, registro, txtSolucao);
                    }
            }
        }
    }
        ...
        throws Exception
    {
        ...
        try
        {
            ...
            if(dependencias.size() == 0 && (declaracao.getModuloDeclaracao().shortValue() == 1 || declaracao.getModuloDeclaracao().shortValue() == 2))
            {
    ...
    {
        int coluna = 3;
        if(dependencia.getOpcaoInscricaoMunicipal().shortValue() == 1 && !InscricaoMunicipal.validarInscMun(dependencia.getCodigoDependencia()))
        {
